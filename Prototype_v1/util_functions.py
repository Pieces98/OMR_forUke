...
"""
def find_best_match_v2(image, templates, scales, search_area_ratio=None):
    if not hasattr(templates, '__iter__'):
        templates = [templates]
...
def find_high_matches_v2(image, templates, scales, threshold, bins=5, min_scale_unit=0.01):
    if not hasattr(templates, '__iter__'):
        templates = [templates]
        
    min_scale, max_scale = scales
    scale_list = np.linspace(min_scale, min_scale_unit, bins)
    if scale_list[0]-scale_list[1] < min_scale_unit:
        scale_list = np.arrange(min_scale, max_scale+min_scale_unit, min_scale_unit)
    score_list = -1*np.ones(scale_list.shape)    
    ...
    for i, template_ in enumerate(templates):
        ...
        search_min_unit = False
        
        while True:
            scale_idx = 0
            while scale_idx<len(sc_list):
                if score_list[scale_idx] < 0:
                    break
                scale_idx += 1
                
            if search_scale == len(sc_list):
                break
